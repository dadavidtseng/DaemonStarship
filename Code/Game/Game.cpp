	  ...
	  m_highScore(0)
{
	...
	const Vec2 worldTopRight  = Vec2(WORLD_SIZE_X, WORLD_SIZE_Y);
	const Vec2 screenTopRight = Vec2(SCREEN_SIZE_X, SCREEN_SIZE_Y);
...
void Game::Render()
{
	if (!m_isAttractMode)
	{
		const Rgba8 clearColor = Rgba8(0.f, 0.f, 0.f);
	...
	if (m_isAttractMode)
	{
		...
		const Vec2 bottomLeft = Vec2(0.f, 0.f);
		const Vec2 topRight   = Vec2(1600.f, 800.f);
...
{
	m_worldCamera->SetOrthoView(m_baseCameraPos, m_baseCameraPos + Vec2(200, 100.f));
}
